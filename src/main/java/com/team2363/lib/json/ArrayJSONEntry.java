 ...
 */
public class ArrayJSONEntry extends JSONEntry {
     ...
     */
    public ArrayJSONEntry(JSONEntry[] array) {
        this.array = array;
    }
    ...
    public String getJSONText(int indentLevel, JSONFormat format) {
        ...
        StringBuilder shortIndentBlock = new StringBuilder(); // this block is for the indent level that the array itself is on
        for (int i = 0; i < indentLevel; i++) {
            shortIndentBlock.append(indentBlock);
        }
        String longIndentBlock = shortIndentBlock.toString() + indentBlock.toString(); // this block is for the indent level that the array items are on

        StringBuilder str = new StringBuilder(); // the JSON text string we are building

        if (format.getArrayBeginOnNewline().value && indentLevel != 0) { // if this is the root entry (indent level is 0) then don't enter down no matter what
            str.append(System.lineSeparator()).append(shortIndentBlock); // if the array should begin on a new line, add a newline and short indent
        }
        str.append("["); // begin array

        String strBetweenItems; // this string goes after the comma and before the next item in arrays. It changes depending on the format
        if (format.getArrayNewlinePerItem().value) {
            strBetweenItems = System.lineSeparator() + longIndentBlock; // if there should be newlines between items, modify strBetweenItms
            str.append(strBetweenItems);
        } else {
            strBetweenItems = " "; // we don't append anything here so there isn't a random space between the open bracket and first item (like this: [ "test", 1])
        }

        str.append(array[0].getJSONText(indentLevel + 1, format)); // add the first item
        for (int i = 1; i < array.length; i++) {
            str.append(",").append(strBetweenItems).append(array[i].getJSONText(indentLevel + 1, format)); // add comma, strBetweenItems, and next item
        }

        if (format.getArrayNewlinePerItem().value) {
            str.append(System.lineSeparator()).append(shortIndentBlock); // only place close bracket ] on newline if each item has been on a new line
        }
