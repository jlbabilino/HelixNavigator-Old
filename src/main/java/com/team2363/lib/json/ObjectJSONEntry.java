 ...
 */
public class ObjectJSONEntry extends JSONEntry {
     ...
     *
     * @param map key-value pairs of <code>String</code> and
     * <code>JSONEntry</code>
     */
    public ObjectJSONEntry(Map<String, JSONEntry> map) {
        this.map = map;
    }
    ...
    public String getJSONText(int indentLevel, JSONFormat format) {
        ...
        StringBuilder shortIndentBlock  = new StringBuilder(); // this block is for the indent level that the object itself is on
        for (int i = 0; i < indentLevel; i++) {
            shortIndentBlock.append(indentBlock);
        }
        String longIndentBlock = shortIndentBlock.toString() + indentBlock.toString(); // this block is for the indent level that the key-value pairs are on
        
        StringBuilder str = new StringBuilder();
        if (format.getObjectBeginOnNewline().value && indentLevel != 0) { // if this is the root entry (indent level is 0) then don't enter down no matter what
            str.append(System.lineSeparator()).append(shortIndentBlock); // if the object should begin on a new line, add a newline and short indent
        }
        ...
        if (format.getObjectNewlinePerItem().value) {
            strBetweenItems = System.lineSeparator() + longIndentBlock; // if there should be newlines, insert them and indent
        ...
        setIterator.forEachRemaining(entry -> { // I use a lambda to avoid having to assign the next item over and over again
